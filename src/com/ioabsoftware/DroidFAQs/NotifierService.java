...
public class NotifierService extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		try {
			...
			Log.d("notif", username + ", " + password + ", " + loginPath + ", " + loginKey);
			...
			r = Jsoup
					.connect(Session.ROOT + "/boards/myposts.php?lp=-1")
					.method(Method.GET)
					.cookies(cookies)
					.timeout(10000)
					.execute();
			...
			if (r.statusCode() != 401) {
				...
				if (lPost != null) {
					...
					if (newDate.getTime() > oldTime) {
						...
						prefs.edit().putLong("notifsLastPost", newTime).commit();
						
						NotificationCompat.Builder notifBuilder =
						        new NotificationCompat.Builder(this)
						        .setSmallIcon(R.drawable.ic_notif_small)
						        .setContentTitle("GameRaven - " + username)
						        .setContentText("New Posts, click to view");

						Intent ampCheck = new Intent(this, AllInOneV2.class);
						ampCheck.putExtra("ampCheck", true)
								.putExtra("ampCheckUser", username);
						...
						pendingAMPCheck = PendingIntent.getActivity(getBaseContext(), 0, ampCheck, 0);
			...
			Toast.makeText(this, "There was an error checking for new messages", Toast.LENGTH_SHORT).show();
			Log.d("notif", "");
			e.printStackTrace();
		}
	}
